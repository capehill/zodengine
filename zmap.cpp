...
void ZMap::LoadPaletteInfo(int terrain_type)
{
	...
	{		 
		...
		// TODO: this will fuck up endian on BE-system because when reading we expect LE
	}
	...
	for (int iter = 0; iter < MAX_PLANET_TILES; iter++)
	{
		palette_tile_info * pti = (palette_tile_info *)((char *)planet_tile_info[i] + iter * sizeof(palette_tile_info));
	
		pti->next_tile_in_effect = SDL_SwapLE16(pti->next_tile_in_effect);
		pti->crater_type = SDL_SwapLE16(pti->crater_type);
	}
...
int ZMap::Read(char* data, int size, bool scrap_data)
{
	...
	basic_info.width = SDL_SwapLE16(basic_info.width);
	basic_info.height = SDL_SwapLE16(basic_info.height);
	basic_info.object_count = SDL_SwapLE16(basic_info.object_count);
	basic_info.zone_count = SDL_SwapLE16(basic_info.zone_count);

	//printf("Width %d, height %d, object count %d, zone count %d\n", basic_info.width, basic_info.height, basic_info.object_count, basic_info.zone_count);
	...
	for(i=0;i<basic_info.zone_count;i++)
	{
		...
		temp_zone.x = SDL_SwapLE16(temp_zone.x);
		temp_zone.y = SDL_SwapLE16(temp_zone.y);
		temp_zone.w = SDL_SwapLE16(temp_zone.w);
		temp_zone.h = SDL_SwapLE16(temp_zone.h);	
	...
	for(i=0;i<basic_info.object_count;i++)
	{
		...
		temp_object.x = SDL_SwapLE16(temp_object.x);
		temp_object.y = SDL_SwapLE16(temp_object.y);
		temp_object.extra_links = SDL_SwapLE16(temp_object.extra_links);
		temp_object.health_percent = SDL_SwapLE32(temp_object.health_percent);
...
#ifdef __amigaos4__		
		temp_tile.tile = SDL_SwapLE16(temp_tile.tile);
#endif
