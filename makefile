CC=ppc-amigaos-g++
CFLAGS=-c -g -O2 `sdl-config --cflags` -D DISABLE_REGCHECK -D DISABLE_MYSQL -D DISABLE_OPENGL 
LDFLAGS=`sdl-config --libs` -lSDL_image -ltiff -ljpeg -lpng -lwebp -lz -lpthread -lSDL_mixer -lSDL_ttf -lvorbisfile -lvorbis -logg -lsmpeg -lflac -lmikmod -lmodplug -lSDL -lm -lfreetype -static -lunix #-lmysqlclient
INCLUDES=
...
main: $(OFILES) 
	$(CC) -o $(EXENAME_NS) $(OFILES) $(LDFLAGS)
	$(STRIP) -o $(EXENAME) $(EXENAME_NS)
...
src:
	tar -cvjf zod_src_${DATE}.tar.bz2 *.cpp *.h makefile
