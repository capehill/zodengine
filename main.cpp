...
int run_player_thread(void *nothing);
void run_tray_app();
...
int run_player_thread(void * nothing)
{
	...
	if(starting_conditions.read_resolution)
		zplayer.SetDimensions(starting_conditions.resolution_width, starting_conditions.resolution_height, starting_conditions.resolution_depth);
...
void display_help(char *shell_command)
{
	...
	printf("-p bitdepth			 - bitdepth (f.ex. 16) used in display\n");
...
void input_options::setdefaults()
{
	...
	printf("no arguments set, using defaults of '-n zlover -t red -r 800x600 -p 32 -w -o'\n");

	read_connect_address = false; //true;
	connect_address = "localhost"; //"hestia.nighsoft.net";
	...
	while ((c = getopt(argc, argv, "c:m:l:n:t:b:z:e:g:i:wr:dhvksuoa")) != -1) 
	{
		switch(c) 
		{
			...
			case 'p':
				...
				resolution_depth = atoi(depth.c_str());
				
				if (resolution_depth == 16 || resolution_depth == 32)
				{
					printf("display depth set to %d\n", resolution_depth);
				}
				else
				{
					printf("invalid display depth %d\n", resolution_depth);
					resolution_depth = 32;
				}
