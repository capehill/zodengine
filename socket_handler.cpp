...
SocketHandler::SocketHandler()
{
	...
	buf = (char *)malloc(MAX_DATA_STORED);
	dp_temp_buf = (char *)malloc(MAX_DATA_STORED);
}
...
int SocketHandler::PacketAvailable()
{
	...
	char * temp_buf = (char *) malloc(MAX_BUF_SIZE);
...
int SocketHandler::SendMessage(int pack_id, const char *data, int size)
{
	...
	if (temp_buf)
	{
		...
		//printf("%s: pack_id: %d, %p, size: %d\n", __FUNCTION__, pack_id, data, size);
...
#warning "GetMac implementation missing"
#else
	int skfd;
	struct ifreq sIfReq;
