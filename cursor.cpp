...
//ZSDL_Surface (* cursor)[MAX_TEAM_TYPES][4];
...
void ZCursor::Exit()
{
	for (int i = 0; i < MAX_CURSOR_TYPES; i++)
	{
		for (int j = 0; j < MAX_TEAM_TYPES; j++)
		{
			for (int k = 0; k < 4; k++)
			{
				cursor[i][j][k].Unload();
...
/*	
	if (cursor == NULL)
	{
		cursor = new ZSDL_Surface[MAX_CURSOR_TYPES][MAX_TEAM_TYPES][4];
	}
*/	
